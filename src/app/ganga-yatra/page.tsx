...
export default function GangaYatraPage() {
  ...
  useEffect(() => {
    const fetchTourData = async () => {
      try {
        ...
        console.log('Ganga Yatra tour found:', gangaYatraTour);
      } catch (error) {
  ...
  const handleBookNow = () => {
    ...
    window.location.href = `/book/${tourData?.id}?passengers=${passengers}`;
  };
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      >
                        ...
                        <option value={tourData?.id}>
                          {new Date(tourData?.departureDate).toLocaleDateString('en-IN', {
                            weekday: 'short',
                            day: 'numeric',
                            month: 'short',
                            year: 'numeric'
                          })} - {tourData?.availableSeats} seats available
                        </option>
                      </select>
                    </div>
