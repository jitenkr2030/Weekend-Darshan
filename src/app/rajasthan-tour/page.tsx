  ...
  useEffect(() => {
    const fetchTourData = async () => {
      try {
        ...
        // Get all Rajasthan tours
        const rajasthanTours = result.data.filter((trip: any) => 
          trip.title.includes('Premium Combo') && 
          trip.title.includes('Khatu Shyam')
        );
        
        // Sort by date
        rajasthanTours.sort((a: any, b: any) => 
          new Date(a.departureDate).getTime() - new Date(b.departureDate).getTime()
        );
        
        setAvailableDates(rajasthanTours);
        
        // Set the first tour as default
        if (rajasthanTours.length > 0) {
          setTourData(rajasthanTours[0]);
          setSelectedDate(rajasthanTours[0].id);
        }
        
        console.log('Rajasthan tours found:', rajasthanTours);
      } catch (error) {
              ...
              <div className="lg:col-span-1">
                <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                  <div className="text-center mb-6">
                    <div className="text-3xl font-bold text-orange-600">₹{tourData?.pricePerSeat || 2000}</div>
